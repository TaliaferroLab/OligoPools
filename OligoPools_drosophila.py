	...
	with open('oligos.dm6.gff', 'w') as outfh:
		for gene in oligocoords:
			...
			for oligo in oligos:
				...
				oligostart = oligo[0]
				oligoend = oligo[1]
				oligooutlist = [chrm, 'dm6', 'oligo', str(oligostart), str(oligoend), '.', strand, '.', 'ID={0}.{1}'.format(gene, oligocounter) + ';gene_id={0}'.format(gene) + ';gene_name={0}'.format(genenames[gene]) + ';oligo_id={0}.{1}'.format(gene, oligocounter) + ';oligo_type=regular' + ';Parent={0}.UTR'.format(gene)]
